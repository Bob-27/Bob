        ...
        if(!videoFile.isEmpty()){
            ...
            if (fileType[0].equals("video")){
                ...
                try {
                      ...
                      //最开始的版本String ossVideoObject=videoDto.getUsername()+"/"+finalFilename;
                      String ossVideoObject = videoDto.getUsername() + "/" + videoDto.getTitle();//视频的

                      //ossClient.putObject(oss.getBUCKET_NAME(), "871821548qqcom/1.mp4", videoFile.getInputStream());
                      //最开始的版本:ossClient.putObject(oss.getBUCKET_NAME(),ossVideoObject, videoFile.getInputStream());
                      //ossClient.putObject(oss.getBUCKET_NAME(), videoDto.getUsername() + "/" + videoDto.getTitle(), videoFile.getInputStream());
                ...
                String imgDir = FFmpegUtils.grabberVideoFramer(finalFilename,myVideoPath.getMYPATH());
                 //设置图片名称
                String imgFileName = videoDto.getTitle() + ".png";
                 //设置图片的oss名称
                String ossImgObject = videoDto.getUsername()+"/"+imgFileName;//图片的
                 //获取视频时长
